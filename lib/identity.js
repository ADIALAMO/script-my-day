...
// ─── Grok Imagine — reference-conditioned image generation ──────────────────────

/**
 * Generates an image from a text prompt conditioned on a reference image URL.
 * Returns a base64 data URI (verified: JPEG). Throws on any failure so callers
 * can fall back / degrade.
 */
export async function grokImageFromReference(prompt, referenceUrl, { timeoutMs = 45000 } = {}) {
  const key = process.env.OPENROUTER_API_KEY;
  if (!key) throw new Error('OPENROUTER_API_KEY not configured');
  if (!referenceUrl) throw new Error('grokImageFromReference requires a referenceUrl');
  ...
  if (!raw) throw new Error('Grok returned no image');
  return raw; // base64 JPEG data URI
}
