...
const nameKey  = (userId) => `referral:name:${userId}`; // referrer's first name (for the /i landing + OG card)

// Canonical absolute base for every shareable URL (single source of truth — keeps OG and
// referral links on the same domain). The invite link points at the dedicated SSR landing
// page /i/<code>, which injects the cookie + renders a cinematic OG preview.
function inviteBase() {
  return (process.env.NEXTAUTH_URL || 'https://lifescript.app').replace(/\/$/, '');
}
export function buildInviteLink(code) {
  return `${inviteBase()}/i/${code}?utm_source=lifescript&utm_medium=referral&utm_campaign=share_loop`;
}
...
export async function getReferralStats(userId) {
  ...
  return { code, link: buildInviteLink(code), referrals, bonusCredits };
}

/**
 * Remember the referrer's first name so the /i/<code> landing page + OG card can greet the
 * invitee personally ("You're invited by Daniel"). Best-effort: called when the user opens
 * their invite UI; a failure just means the card falls back to a generic greeting.
 */
export async function rememberReferrerName(userId, fullName) {
  const first = (fullName || '').trim().split(/\s+/)[0];
  if (!first) return;
  try {
    await redis.set(nameKey(userId), first);
  } catch {
    /* non-critical — landing page degrades to a generic greeting */
  }
}

/**
 * Resolve an invite code to its referrer for the /i/<code> landing page.
 * Returns { userId, name } — name is '' when unknown. Unknown code → null (generic invite).
 */
export async function lookupReferrer(code) {
  if (!code || !/^[a-z0-9]{1,16}$/i.test(code)) return null;
  try {
    const userId = await redis.get(`referral:code:${code}`);
    if (!userId) return null;
    const name = (await redis.get(nameKey(userId))) || '';
    return { userId: String(userId), name: String(name) };
  } catch {
    return null; // Redis down → treat as generic invite (still cookie-captured client-side)
  }
}
