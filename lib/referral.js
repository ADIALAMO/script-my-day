...
// The invite link points at the dedicated SSR landing page /i/<code>, which injects the
// cookie + renders a cinematic OG preview. Base comes from the shared SITE_URL (lib/site.js)
// so OG and referral links always live on the same live domain.
export function buildInviteLink(code) {
  return `${SITE_URL}/i/${code}?utm_source=lifescript&utm_medium=referral&utm_campaign=share_loop`;
}
