...
// TEMPORARY: identifies one serverless-container cold start, so log lines
// across requests reveal whether they share a warm instance or each came
// from a fresh cold start. buildAdapter() runs once at module-eval time
// (adapter: buildAdapter() is a plain object literal), so if this ever
// resolves differently across instances, this ID pinpoints which instance
// made which decision. REMOVE once root-caused.
export const INSTANCE_ID = Math.random().toString(36).slice(2, 8);

function buildAdapter() {
  const url   = process.env.UPSTASH_REDIS_REST_URL;
  const token = process.env.UPSTASH_REDIS_REST_TOKEN;
  console.error(`[AUTH-DEBUG][instance:${INSTANCE_ID}] buildAdapter: url=${url ? 'SET(' + url.length + ')' : 'MISSING'} token=${token ? 'SET(' + token.length + ')' : 'MISSING'}`);
  if (!url || !token) {
...
export const authOptions = {
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        try {
          ...
          // GETDEL is atomic: read and delete happen in a single Redis command,
          // closing the TOCTOU race window where two concurrent signIn calls
          // could both read the token before either deletes it — which would
          // produce two valid sessions from one magic-link click.
          const raw = await redis.getdel(`relay:${relayToken}`);
          if (!raw) return null;
  ...
  // Chasing a real "Configuration" error that only reproduces via the actual
  // Capacitor WebView (never via curl replication of the same request/cookie/
  // header sequence). NextAuth's default logger swallows the underlying Error
  // object's message/stack before it reaches Vercel's function logs — this
  // overrides it to surface the full detail. REMOVE once root-caused.
  logger: {
    error(code, metadata) {
      const err = metadata?.error instanceof Error ? metadata.error : metadata;
  ...
  events: {
    async signIn({ user, account, isNewUser }) {
      ...
      try {
        ...
        // Cumulative signup counter for the admin dashboard (/api/admin/stats).
        // Fires once per new account (isNewUser is adapter-driven). Best-effort.
...
// ── VIP Pro allowlist ─────────────────────────────────────────────────────────
// PRO_ALLOWLIST = comma-separated emails that receive Pro on sign-in WITHOUT
// going through Stripe. Used to hand invited influencers / key testers Pro while
// public billing is gated behind a waitlist (NEXT_PUBLIC_BILLING_ENABLED=false).
// Access is bound to identity, so unlike a shareable ?beta= link or access code
// it cannot leak: a leaked address grants no one else anything.
function isProAllowlisted(email) {
  if (!email || !process.env.PRO_ALLOWLIST) return false;
  const allowed = process.env.PRO_ALLOWLIST
    .split(',')
...
export async function getSessionAndTier(req, res) {
  ...
  if (session?.user?.id) {
    ...
    // Lift allowlisted VIPs free → pro. Never downgrades a stored admin/pro.
    if (tier === 'free' && isProAllowlisted(session.user.email)) tier = 'pro';
